...
function makeM3(overrides: Partial<M3_Output> = {}): M3_Output {
  return {
    ...
    course_profile_id: "profile-1",
    segments: [
      { segment_index: 0, concept_keys: ["c1", "c2"], cognitive_function: "encode", duration_sec: 120, is_anchor: true },
      { segment_index: 1, concept_keys: ["c3"], cognitive_function: "consolidate", duration_sec: 60, is_anchor: false },
    ],
    concept_order: ["c1", "c2", "c3"],
    repetition_plan: [
      { concept_key: "c1", moments: ["inline", "final_test"], is_critical: true },
    ],
    mnemonics: [{ concept_keys: ["c1"], mnemonic: "C1 = Core One", type: "acronym" }],
    visual_anchors: [{ concept_key: "c1", content: "A tree representing c1", anchor_type: "image" }],
    cognitive_budget: { max_new_per_segment: 5, total_segments: 2, estimated_total_duration_sec: 180 },
    ...
    const input = makeInput({
      m2_output: makeM2({
        confidence: {
          ...
          traps: 0.6,
          structure: 0.7,
          ambiguous_zones: [{ zone_label: "Section 3", reason: "unclear" }],
        },
      }),
    });
    const output = generateDynamicSheetLocally(input);
